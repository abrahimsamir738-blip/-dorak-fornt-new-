...
// --- مكون التنبيه المخصص (Custom Modal) ---
   ...
   {isOpen && (
    ...
    >
     <motion.div
      ...
      className="bg-white dark:bg-slate-900 rounded-[40px] p-8 md:p-12 max-w-lg w-full shadow-2xl border border-slate-100 dark:border-slate-800 text-center relative overflow-hidden"
     >
      <button onClick={onClose} className="absolute top-6 left-6 text-slate-400 hover:text-slate-600 transition-colors">
       <X size={24} />
      </button>
      ...
      <p className="text-slate-500 dark:text-slate-400 text-lg md:text-xl font-bold leading-relaxed mb-10">
       نعمل حالياً على تطوير نظام تنبيهات ذكي يرسل لك إشعاراً عبر <span className="text-emerald-500">WhatsApp</span> عندما يقترب دورك بـ مريضين.
      </p>
...
const QueueDashboard: React.FC<QueueDashboardProps> = ({
  ...
  const fetchQueueData = async () => {
   if (clinicId) {
    try {
     ...
     setCurrentTurn(queueData.current_serving_number || 1);
    } catch (error) {
     console.error('فشل في جلب بيانات الدور:', error);
    }
   } else {
 ...
 const handleReschedule = () => {
  navigate(`/doctors/${doctorName}?reschedule=${bookingId}`);
 };

 const handleOpenMaps = () => {
  const url = (lat && lng)
   ? `https://www.google.com/maps?q=${lat},${lng}`
   : `https://www.google.com/maps/search/${encodeURIComponent(clinicName + ' ' + (doctorLocation || ''))}`;
 ...
 return (
  <div className="flex flex-col items-center gap-8 md:gap-16 w-full max-w-6xl mx-auto py-10 md:py-20 px-4" dir="rtl">
    ...
    <div className="flex flex-col md:flex-row justify-center items-center gap-10 md:gap-24 mb-12 md:mb-24">
     ...
     <div className="text-center w-full md:w-auto">
      <div className="bg-white/5 rounded-[40px] md:rounded-[80px] p-8 md:p-16 border border-white/5 backdrop-blur-2xl shadow-inner relative">
       ...
       <p className="text-blue-500 text-lg md:text-xl font-black mt-6 tracking-widest uppercase">{currentStatus}</p>
      </div>
     </div>
    </div>

    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 px-4 md:px-10">
     <StatBox icon={<Clock size={32} />} label="الانتظار المتوقع" value={`~ ${waitTime} دقيقة`} color="text-blue-400" />
     <StatBox icon={<Activity size={32} />} label="الحالات المتبقية" value={`${patientsAhead} حالات`} color="text-emerald-400" />
     <StatBox icon={<ShieldCheck size={32} />} label="حالة الحجز" value={currentStatus} color="text-blue-400" />
    </div>
   </motion.div>

   {/* كروت التفاعل السفلية */}
   <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 w-full">

    {/* كارت التنبيه الذكي */}
    <div className="bg-blue-600 rounded-[40px] md:rounded-[80px] p-10 md:p-16 text-white flex flex-col items-center text-center shadow-2xl relative overflow-hidden group transition-all duration-500">
    ...
    {/* كارت الموقع */}
    <div className="bg-white dark:bg-slate-900 rounded-[40px] md:rounded-[80px] p-10 md:p-16 flex flex-col items-center justify-between shadow-xl border border-slate-100 dark:border-slate-800 relative group">
     <div className="flex flex-col items-center gap-6 mb-10">
      ...
      <div className="text-center">
       <h4 className="text-2xl md:text-4xl font-[950] text-slate-900 dark:text-white tracking-tighter mb-1">{doctorName}</h4>
     ...
     <div className="flex flex-col w-full gap-4">
      ...
      {['منتظر', 'قيد الكشف'].includes(currentStatus) && (
       <div className="flex gap-4 mt-2">
        <button onClick={handleReschedule} className="flex-1 bg-amber-50 text-amber-600 font-black py-5 rounded-[25px] flex items-center justify-center gap-2 hover:bg-amber-100 transition-colors">
         <Calendar size={20} /> تأجيل
        </button>
        <button disabled={isCancelling} onClick={handleCancelBooking} className="flex-1 bg-rose-50 text-rose-600 font-black py-5 rounded-[25px] flex items-center justify-center gap-2 hover:bg-rose-100 transition-colors">
         {isCancelling ? <Loader2 size={20} className="animate-spin" /> : <Trash2 size={20} />} إلغاء
        </button>
       </div>
      )}
     </div>
   ...
   <button className="mt-8 text-slate-400 dark:text-slate-600 font-black text-xl md:text-2xl hover:text-blue-600 transition-colors flex items-center gap-4 group" onClick={handleBackNavigation}>
    <ArrowRight size={24} className="group-hover:translate-x-2 transition-transform" />
    <span>العودة للملف الشخصي</span>
   </button>

   {/* استدعاء المودال الجميل */}
   <UpgradeAlert
...
const StatBox = ({ icon, label, value, color }: { icon: any, label: string, value: string, color: string }) => (
 <div className="flex flex-row-reverse items-center justify-center md:justify-start gap-6">
  <div className={`w-16 h-16 md:w-20 md:h-20 bg-white/5 rounded-2xl flex items-center justify-center border border-white/5 ${color}`}>
   {icon}
  </div>
  <div className="text-right">
   <p className="text-slate-500 text-xs md:text-sm font-black uppercase mb-1 tracking-widest">{label}</p>
