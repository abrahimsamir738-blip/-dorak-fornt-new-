...
const BookingModal: React.FC<BookingModalProps> = ({ isOpen, onClose, doctor, onConfirm }) => {
 ...
 useEffect(() => {
  ...
  if (isDark) {
   if (nameInputRef.current) nameInputRef.current.style.webkitTextFillColor = 'white';
   if (phoneInputRef.current) phoneInputRef.current.style.webkitTextFillColor = 'white';
  }
 }, []);
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
           ...
           return (
            <button
             ...
             className={`p-4 rounded-2xl border-2 text-right transition-all relative ${isClosed
              ? 'border-rose-300 bg-rose-50 dark:bg-rose-900/20 opacity-75 cursor-not-allowed'
              : isSelected
            ...
            >
             <div className="flex justify-between items-center">
              <div>
               <p className={`font-black ${isClosed ? 'text-rose-600 dark:text-rose-400' : 'text-slate-900 dark:text-white'}`}>{branch.name}</p>
               <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{branch.address}</p>
              </div>
              <MapPin size={18} className={isSelected ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500'} />
             </div>
            </button>
           );
          })
         ) : (
          <p className="text-center text-slate-400 dark:text-slate-500">لا توجد عيادات</p>
         )}
        </div>
       </div>
       ...
       {selectedDay !== null && (
        <div>
         ...
         {isLoadingSlots ? (
          <div className="flex justify-center py-8"><Loader2 className="animate-spin text-blue-600 dark:text-blue-400" size={32} /></div>
            ...
            return (
             <button
              ...
              className={`p-4 rounded-xl border-2 text-center transition-all ${isSelected
               ...
               : isFull
                ? 'opacity-50 cursor-not-allowed bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700'
                : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white hover:border-blue-400'
               }`}
             >
       ...
       {/* البيانات الشخصية - تحسين الوضوح في الوضع المظلم */}
       <div className="grid md:grid-cols-2 gap-4">
        <div className="space-y-2">
         ...
         <div className="relative">
          <User className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" size={18} />
          <input
           type="text"
           required
           value={name}
           onChange={(e) => setName(e.target.value)}
           className="w-full pr-12 pl-4 py-4 rounded-xl border-2 border-slate-200 dark:border-white bg-white dark:bg-transparent text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 outline-none focus:border-blue-600 dark:focus:border-blue-500 transition-all font-bold [&:-webkit-autofill]:bg-transparent [&:-webkit-autofill]:dark:[-webkit-text-fill-color:white] [&:-webkit-autofill]:shadow-[0_0_0px_1000px_transparent_inset] dark:[&:-webkit-autofill]:shadow-[0_0_0px_1000px_#0f172a_inset]"
        ...
        <div className="space-y-2">
         ...
         <div className="relative">
          ...
          <input
           ...
           className="w-full pr-12 pl-4 py-4 rounded-xl border-2 border-slate-200 dark:border-white bg-white dark:bg-transparent text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 outline-none focus:border-blue-600 dark:focus:border-blue-500 transition-all font-bold"
       ...
       <button
        ...
        className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-slate-200 dark:disabled:bg-slate-700 text-white py-5 rounded-2xl font-black text-lg shadow-xl transition-all active:scale-95 flex items-center justify-center gap-3"
       >
