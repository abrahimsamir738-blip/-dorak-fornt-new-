...
import { X, User, Phone, ShieldCheck, CheckCircle2, ArrowRight, Loader2, Lock, Eye, EyeOff } from 'lucide-react';
...
const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, onLoginSuccess }) => {
 ...
 const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
   const endpoint = isLoginView ? '/api/loginUser' : '/api/createUser';
    ...
    <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
     ...
     >
      ...
      <AnimatePresence>
       {isSuccess && (
        ...
        >
         ...
         <p className="text-blue-100 font-bold text-lg">مرحباً بك في منصة عياداتي كير
         </p>
        </motion.div>
       )}
      </AnimatePresence>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 flex flex-col items-center w-full">
       ...
       {/* حقل كلمة المرور */}
       <div className="space-y-2 w-full">
        <label className="text-xs font-black text-slate-400 mr-2 uppercase tracking-widest block text-right">كلمة المرور</label>
        <div className="relative">
         ...
         <input
          ...
          placeholder="••••••••"
          className="w-full bg-slate-50 dark:bg-slate-800/40 border-2 border-transparent focus:border-blue-600 rounded-2xl py-4 pr-12 pl-12 text-right font-bold outline-none transition-all"
         />
         <button
          type="button"
       ...
       {error && (
        <motion.p
         initial={{ opacity: 0 }}
         animate={{ opacity: 1 }}
         className="text-red-500 text-xs font-bold text-center bg-red-50 dark:bg-red-900/20 p-3 rounded-xl w-full"
        >
         {error}
        </motion.p>
       )}
